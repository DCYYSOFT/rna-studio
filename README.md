...
| **排版** | 选中整个 stem 分支 | — | 拖本体平移；拖圆点绕连接点旋转 |
...
`Cmd/Ctrl+Z` 撤销、`Cmd/Ctrl+Shift+Z` 重做、`Delete` 解除选中碱基的配对、
...
- 拖上方的圆点 → 绕连接点**整体旋转整个子树**（stem + 下游全部结构）。
  这是刚体变换：stem 内部间距、配对、ΔG 全部不变；按 **Shift** 吸附到 15° 整数倍
- 拖动本体 → 整个分支**刚体平移**；与其它 stem 中心对齐时自动吸附并显示辅助线
- **双击** stem → 重置该 stem 的角度
- **右键** stem → 重置整个分支（子树内全部调整）
- 旋转 / 平移只是**排版**：不改 dot-bracket、不改配对、不重新折叠、ΔG 不变

排版结果以**语义状态**保存（每个 stem 的角度 / 位移，而不是每个碱基的坐标），
...
看结构或给别人演示时不想误改画布，把编辑模式切到「仅预览」即可。
...
在「改序列」模式下，**双击**结构上或序列条上的任意碱基，会弹出 A/U/G/C
选择器直接替换。改完会立刻检查所有配对是否仍然合法（A–U、G–C、G–U），
不再合法的配对会标成紫色点线提示——但**不会自动解除**，你可能只是想试试这个错配。
...
序列、约束、手工搭的结构、结构域标注、界面参数（含当前编辑模式）都会自动存在本地，
