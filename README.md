...
画布上的一切交互由工具栏的**编辑模式**开关决定。分开之后每个模式下点击的含义都唯一，
不会出现「想拖动却建了一对配对」这种事：

| 模式 | 单击碱基 | 双击碱基 | 拖动 |
|---|---|---|---|
| **仅预览** | 无反应（画布锁定） | 无反应 | 平移画布 |
| **改配对** | 选中；再点另一个则建立配对 | — | 平移画布 |
| **改序列** | 只高亮定位，不建配对 | 弹出 A/U/G/C 替换字母 | 平移画布 |
| **排版** | 选中它所在的螺旋/环 | — | 拖动选中项；拖圆点旋转 |

各模式的补充操作：

- **改配对**：右键点碱基解除配对；Alt+点击 / Shift+点击 标记「禁止配对」（红虚线圈）
- **改序列**：改完自动检查配对是否仍合法，不合法的标成紫色点线，但**不会自动解除**
- **排版**：见下节

通用操作：滚轮缩放、空白处拖动平移、双击空白恢复适应窗口。
`Cmd/Ctrl+Z` 撤销、`Cmd/Ctrl+Shift+Z` 重做、`Esc` 取消选中、`Cmd/Ctrl+Enter` 折叠。

结构变化时会有一段**弹性过渡动画**：碱基从旧位置带回弹地滑到新位置。

### 排版模式（BioRender 式对象操作）

点中一个螺旋或环，会出现虚线选择框和上方的**旋转手柄**：

- 拖动本体 → 整体平移，与之相连的环平滑跟着变形
- 拖上方的圆点 → 绕自身质心旋转，每 15° 吸附一次
- 拖动时与其它螺旋中心对齐会自动吸附，并显示琥珀色辅助线
- **断开为自由图形** → 在该段两端打断骨架，从此移动旋转不再牵引相邻的环，
  可以像摆图形一样自由摆放；**碱基配对与序列完全不变**，随时可「重新连接」还原

排版结果会保留并随图导出，点「恢复自动布局」可还原。
